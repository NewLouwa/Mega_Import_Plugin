...
- **Self-healing queue**: if the worker is killed mid-import (restart / reboot / crash), any stale `downloading` item is requeued and the worker is respawned automatically on the next `queue_status` poll or `enqueue` — the queue resumes on its own. (Byte-level resume of a single interrupted file isn't possible with mega.py, so that file restarts from zero.)
- **Server-side orphan cleanup**: the worker prunes `megapy_*` temp + staging leftovers older than 1 h on startup (active downloads keep a fresh mtime, so they're never touched) — so cleanup happens even with the browser closed, not only when the UI polls `temp_progress`.
